...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\MasterCook_11_Setup.exe" -Parameters "/s /f1`"$dirSupportFiles\setup.iss`"" -WindowStyle "Hidden" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-Process -Path "C:\Program Files (x86)\InstallShield Installation Information\{25CA5771-2536-4D47-A12F-E9AF3B5ADB81}\setup.exe" -Parameters "-runfromtemp -l0x0009 -removeonly" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
